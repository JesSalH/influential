...
/** Checks that a Windows Path entry survives when the wrapper adds local binaries. */
test(
    "preserves Windows Path when launching node by name",
    ...
    async () => {
        ...
        const { stdout } = await execFileAsync(process.execPath, [WRAPPER, "node", "--version"], {
            env: environment,
        });
